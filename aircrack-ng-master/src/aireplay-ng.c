...
#define BEACON_REQ                                                              \
	"\x80\x00\x00\x00\xFF\xFF\xFF\xFF\xFF\xFF\x00\x6C\xBC\x0D\x3D\x92"         \
	"\x00\x6C\xBC\x0D\x3D\x92\x32\xA8"
...
static int send_hello_packet(void)
{
	int i, n;
	...
	while (1)
	{
		...
		if (memcmp(opt.r_dmac, NULL_MAC, 6) != 0)
		{
			...
			memcpy(h80211, BEACON_REQ, 26);
			memcpy(h80211 + 16, opt.r_bssid, 6);
			
			memcpy(h80211 + 24,&abc,2);
			...
			for (i = 0; i < 64; i++)
			{
				...
				{
					PCT;
					printf("Sending 64 directed Beacon (code %i). STMAC:"
				...
				while (1)
				{
					...
					if (tmpbuf[0] == 0xD4)
					{
						...
						PCT;
						printf(
							"Sending 64 directed Beacon (code %i). STMAC:"
							" [%02X:%02X:%02X:%02X:%02X:%02X] [%2d|%2d ACKs]\r",
							opt.deauth_rc,
							opt.r_dmac[0],
							opt.r_dmac[1],
		...
		else
		{
			...
			PCT;
			printf("Sending Beacon (code %i) to broadcast -- BSSID:"
			...
			memcpy(h80211, BEACON_REQ, 24);
			h80211[24] = opt.deauth_rc;

			memcpy(h80211 + 4, BROADCAST, 6);
			memcpy(h80211 + 10, opt.r_bssid, 6);
			memcpy(h80211 + 16, opt.r_bssid, 6);
			memcpy(h80211+22,&abc,2);
